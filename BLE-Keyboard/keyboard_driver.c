...
{
    nrf_gpio_cfg(LED_CAPS, NRF_GPIO_PIN_DIR_OUTPUT, NRF_GPIO_PIN_INPUT_DISCONNECT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_S0D1, NRF_GPIO_PIN_NOSENSE);
    nrf_gpio_cfg(LED_FN, NRF_GPIO_PIN_DIR_OUTPUT, NRF_GPIO_PIN_INPUT_DISCONNECT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_S0D1, NRF_GPIO_PIN_NOSENSE);

    nrf_gpio_pin_clear(LED_CAPS);
    nrf_gpio_pin_clear(LED_FN);
}
...
static bool keymatrix_read(uint8_t *pressed_keys, uint8_t *number_of_pressed_keys)
{
    ...
    *number_of_pressed_keys = 0;
    ...
    for (uint_fast8_t column =  KEYBOARD_NUM_OF_COLUMNS; column--;){	// Loop through each column
        nrf_gpio_pin_set((uint32_t) column_pin_array[column]);          // set col to high
